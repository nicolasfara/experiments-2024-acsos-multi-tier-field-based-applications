    ...
    body = device => {
      if (param == "send") {
        ...
        if (scafiSurrogateProgramsList.isEmpty) {
          throw new IllegalStateException(
            "There is no program requiring a " + classOf[SendSurrogateScafiMessage[T, P]].getSimpleName + " action"
          )
        }
        if (scafiSurrogateProgramsList.size > 1) {
          throw new IllegalStateException(
            "There are too many programs requiring a " + classOf[
              SendSurrogateScafiMessage[T, P]
            ].getName + " action: " + scafiSurrogateProgramsList
          )
        }
        new SendSurrogateScafiMessage[T, P](environment, device, reaction.asInstanceOf[Reaction[T]], scafiSurrogateProgramsList.head)
      } else {
...
object CachedInterpreter {
  ...
  /** Evaluates str using Scala reflection. When doCacheValue is true, the result of the evaluation is cached. When doCacheValue is false, only the
    * result of parsing is cached.
    */
